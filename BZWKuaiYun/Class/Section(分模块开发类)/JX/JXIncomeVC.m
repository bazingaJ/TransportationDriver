...
static const NSInteger kPickerViewHeight = 250;

@interface JXIncomeVC ()<JXDateChoiceDelegate,JXDatePickerDelegate>
...
@implementation JXIncomeVC

- (void)viewDidLoad
{
    [super viewDidLoad];
    [self setupUI];
    [self requestMyInOutComeWithYearMonthString:[JXAppTool getNowYearAndMonth]];
}
...
- (void)requestMyInOutComeWithYearMonthString:(NSString *)str
{
    SVSHOW
    [JXRequestTool postMyInOutComeNeedPageSize:@"10" pageNum:@"1" queryDate:str type:@"1" complete:^(BOOL isSuccess, NSDictionary *respose) {
        if (isSuccess)
        {
            ...
            NSString *incomeStr = [NSString stringWithFormat:@"%@",total];
            
            self.headerView.wholeMoneyLab.text = [incomeStr stringByReplacingOccurrencesOfString:@".00" withString:@""];
...
- (void)yearButtonClick
{
    ...
    [UIView animateWithDuration:.03f animations:^{
        ...
        self.picker.transform = CGAffineTransformMakeTranslation(0, -kPickerViewHeight);
    }];
}
...
- (JXDatePickerView *)picker
{
    if (!_picker)
    {
        _picker = [[JXDatePickerView alloc] initWithFrame:CGRectMake(0, Main_Screen_Height-64, Main_Screen_Width, kPickerViewHeight)];
...
- (void)containButtonClickAfter:(NSString *)str
{
    ...
    [self requestMyInOutComeWithYearMonthString:[str stringByReplacingOccurrencesOfString:@" " withString:@"-"]];
}
