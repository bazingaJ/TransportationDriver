...
@interface JXTool ()<UIAlertViewDelegate>
...
+ (BOOL)verifyPlateNum:(NSString *)string
{
    ...
    JXLog(@"carTest is %@",carTest);
    if ([carTest evaluateWithObject:string])
    {
        return YES;
    }
    else
...
{
    if ((str == nil || [str isKindOfClass:[NSNull class]] || [str isEqual:[NSNull null]] ||str.length == 0))
    {
        return @"密码不能为空";
    }
    else
    {
        ...
        if (length < before)
        {
            NSString *alertStr = [NSString stringWithFormat:@"密码长度需大于%ld位",(long)before];
            return alertStr;
        }
        else
        {
            if (length > end)
            {
                NSString *alertStr = [NSString stringWithFormat:@"密码长度需小于%ld位",(long)end];
                return alertStr;
            }
            else if ([JXTool hasillegalString:str])
            {
                return @"密码不能含有非法字符";
            }
            else
...
{
    CGSize size = [str sizeWithAttributes:@{NSFontAttributeName:F12}];
    CGFloat wid = size.width;
    NSInteger row = wid / width;
    if (row > 0)
    {
        
    }
    CGFloat height = size.height * (row+1);
    return height;
}
